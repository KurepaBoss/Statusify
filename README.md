<div align="center">
  ...
  <h1>Statusify v1.1.6</h1>
  <p><strong>The ultimate Discord Rich Presence & Spotify Lyrics bridge.</strong></p>

  ![Statusify v1.1.6](https://img.shields.io/badge/Statusify-v1.1.6-brightgreen?style=for-the-badge)
...
- **Hotkeys:** Default `Ctrl+Alt+S` to toggle Discord RPC, `Ctrl+Alt+N` to skip tracks.
- **Theme:** Smooth Dark and Light modes with custom accent colors.
- **History:** Toggle session recording on/off in the Settings menu.
- **Lyric Delay:** Fine-tune sync timing with `lyric_delay_ms`.

---

## 🛠️ Manual Spicetify Extension Install

If the automatic install doesn't work, see [INSTALL.md](INSTALL.md) for manual steps.

**Quick summary:**
1. Copy `lyrics-bridge.js` to `%appdata%\spicetify\Extensions\`
2. Run:
   ```
